...
import { AdminSchema, SupportSchema } from "../Constants/Schema";
...
function SupportLogin() {
  ...
  const onSubmit = (values) => {
    if (values.email === "supportcinestream" && values.password === "support123") {
      if (!isToastVisible) {
      ...
      localStorage.setItem("supportId", 1);
    } else if (values.email === "supportcinestream") {
      if (!isToastVisible) {
        setToastVisible(true);
    ...
    } else if (values.password === "support123") {
      if (!isToastVisible) {
        setToastVisible(true);
        toast.warning("Username Mismatch", {
          onClose: () => setToastVisible(false),
        });
      }
    } else {
